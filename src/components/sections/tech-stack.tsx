    ...
    <section className="py-24 relative overflow-hidden bg-white">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 mb-12">
        <div className="text-center animate-fade-in-up">
          ...
          <h2 className="text-3xl sm:text-4xl font-bold tracking-tight text-[#33092B]">
            Our Tech Stack
          </h2>
        </div>
      </div>
      ...
      <div className="relative mb-4">
        ...
        <div className="flex gap-3 w-max animate-marquee">
          {[...row1, ...row1].map((tech, i) => (
            <div
              key={`r1-${i}`}
              className="flex items-center gap-2 px-5 py-2.5 rounded-full border border-[#E3D3E0] bg-[#FAF4F9] whitespace-nowrap hover:border-[#C99BC1] hover:bg-[#F0E4EE] transition-colors cursor-default"
            >
              <span className="text-sm">{tech.emoji}</span>
              <span className="text-sm text-[#96568B] font-medium">{tech.name}</span>
            </div>
          ))}
        </div>
      </div>
