...
export function Stats() {
  return (
    <section className="py-20 relative overflow-hidden bg-[#000]">
      {/* Subtle pattern */}
      <div className="absolute inset-0 bg-[radial-gradient(rgba(255,255,255,0.07)_1px,transparent_1px)] bg-[size:24px_24px]" />
      <div className="absolute top-0 left-1/4 w-96 h-96 bg-[#111]/40 rounded-full blur-[100px] pointer-events-none" />
      <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-[#555]/30 rounded-full blur-[100px] pointer-events-none" />
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
          {stats.map((stat, i) => (
            ...
            >
              ...
              <div className="font-semibold text-sm text-[#e5e5e5] mb-1">{stat.label}</div>
              <div className="text-xs text-[#aaa] leading-relaxed">{stat.description}</div>
            </div>
          ))}
        </div>
      </div>
