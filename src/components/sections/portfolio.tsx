...
export function Portfolio() {
  return (
    <section id="portfolio" className="py-28 lg:py-36 relative bg-[#faf8fd]">
          ...
          {projects.map((project, i) => (
            ...
            >
              <div className="relative h-52 overflow-hidden">
                ...
                />
                <div className="absolute inset-0 bg-linear-to-t from-[#170b33]/40 via-transparent to-transparent" />
              ...
              <div className="flex flex-col flex-1 p-6">
                <h3 className="font-bold text-lg mb-2 text-[#170b33]">{project.title}</h3>
                <p className="text-sm text-[#5d4483] leading-relaxed mb-4 flex-1">{project.description}</p>
                <div className="flex flex-wrap gap-1.5 mb-4">
                  {project.tags.map((tag) => (
                    <span key={tag} className="px-2.5 py-0.5 rounded-full text-xs border border-[#e4dbf5] bg-[#faf8fd] text-[#5d4483]">
                      {tag}
                    </span>
                  ))}
                </div>
                <div className="flex items-center justify-between pt-4 border-t border-[#e4dbf5]">
                  <span className="text-xs font-semibold text-[#170b33]">✦ {project.result}</span>
                  <Link href="#contact" className="inline-flex items-center gap-1 text-xs text-[#5d4483] hover:text-[#170b33] transition-colors font-medium">
